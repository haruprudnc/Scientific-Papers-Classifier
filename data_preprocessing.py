...
FEATURE_COLS = ['title', 'authors', 'authors_count_token', 'abstracts', 'venue', 'year_token', 'primary_topic', 'keywords']
...
def train_model(df, target):
    print("--- Starting Robust Voting Classifier Training ---")
    ...
    clf_lr = LogisticRegression(C=2.0, class_weight='balanced', max_iter=2000, solver='lbfgs', random_state=42)
    ...
    clf_hgb = HistGradientBoostingClassifier(random_state=42) # Extremely fast for dense data
    ...
    voting_clf = VotingClassifier(
        ...
        voting='soft', # Soft voting utilizes the probability predictions for better ensembling
        n_jobs=-1
    )
    
    # 8. Train the ensemble
    print("Training the Voting Classifier (this may take some time)...")
    voting_clf.fit(X_train, y_train)
    
    # 9. Evaluate and print results
    ...
    # joblib.dump(voting_clf, 'voting_classifier_head.pkl')
    
    print("--- Model Training Completed ---")
    return voting_clf, embedder

# .-. ======================================== PREDICTION ======================================== .-. #
def predict(file_path, clf, embedder):
    print(f"--- Generating Predictions for {file_path} ---")
...
if __name__ == "__main__":
    ...
    # Train the robust ensemble model
    voting_clf, embedder = train_model(df, target)
    
    # Generate predictions on the test set
    predict(TEST_PREPROCESS_PATH, voting_clf, embedder)
